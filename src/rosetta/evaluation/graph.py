...
ARM_VARIANTS: list[tuple[str, str, str]] = [
    # (label, processor_dir, slaspec_filename)
...
def compare_all_variants(
    ...
    """
    Compare a generated .slaspec against every ARM/AARCH64 variant in Ghidra.
    Returns a list of result dicts (one per variant), sorted by instruction_coverage desc.
    """
    from rosetta.evaluation.spec_loader import load_slaspec_text
...
def plot_variant_comparison(
    ...
    """Grouped bar chart: instruction coverage + register overlap per ARM variant."""
    ...
    if has_sem:
        bars3 = ax.bar(x + width,   sem_sim,     width, label="Semantic similarity", color="#FF9800")
