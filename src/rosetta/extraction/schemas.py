...
class InstructionDef(BaseModel):
    ...
    bit_fields: dict[str, str] = Field(
        ...
        description="Bit field definitions: field_name -> 'high:low' bit range (e.g. 'opcode': '31:28')",
    )
    bit_constraints: dict[str, str] = Field(
        default_factory=dict,
        description="Required bit values: field_name -> binary string (e.g. 'opcode': '0000')",
    )
    ...
    pcode_hint: str = Field(
        default="",
        description="SLEIGH P-code approximation of the semantics, e.g. 'Rd = Rn + Rm;'",
    )
